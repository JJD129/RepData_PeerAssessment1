...
fd$day <- as.factor(sapply(fd$date, day))
fd$daydate <- sapply(fd$date, FUN = day)
...
```{r, echo=FALSE Results=hide}
averages <- aggregate(steps ~ interval + day, data = fd, mean)
ggplot(averages, aes(interval, steps)) + geopm_line() + facet_grid(day~.) + xlab("5-minute interval") + ylab("Number of steps")
```